    ...
    function(window2, registerGlobal) {
      ...
      class CrossWord {
          ...
          if (!IS_MOBILE) {
            this.top_text.on("contextmenu", (e) => {
              ...
              if (!selectedText.trim()) {
                ...
                const onlyText = (_a = topClone.querySelector) == null ? void 0 : _a.call(topClone, ".cw-clue-text");
        ...
        async openDucktilesOverlayWithClipboard(rawText) {
          ...
          const isMac = (_a = navigator.platform) == null ? void 0 : _a.toLowerCase().includes("mac");
        ...
        `;
          ...
          (_b = modalContent == null ? void 0 : modalContent.classList) == null ? void 0 : _b.add("modal-large");
          (() => {
            const modalEl2 = this.root.find(".cw-modal").get(0);
            const box = modalContent;
            if (!modalEl2 || !box) return;
            box.classList.add("modal-draggable");
            box.style.position = "fixed";
            if (!box.dataset.positioned) {
              const vw = window2.innerWidth, vh = window2.innerHeight;
              const bw = box.offsetWidth, bh = box.offsetHeight;
              const left = Math.max(8, Math.round((vw - bw) / 2));
              const top = Math.max(8, Math.round((vh - bh) / 10));
            ...
            const onMouseMove = (e) => {
              ...
              const GUARD_X = 28;
              const GUARD_Y = 28;
              const vw = window2.innerWidth;
              const vh = window2.innerHeight;
            ...
            const endDrag = () => {
              ...
              window2.removeEventListener("mouseup", endDrag, true);
            };
            const onMouseDown = (e) => {
              ...
              window2.addEventListener("mouseup", endDrag, true);
              e.preventDefault();
              e.stopPropagation();
            };
            header.addEventListener("mousedown", onMouseDown, true);
            const observer = new MutationObserver(() => {
              if (modalEl2.style.display === "none") {
                endDrag();
                observer.disconnect();
              }
            });
            observer.observe(modalEl2, {
              attributes: true,
              attributeFilter: ["style"]
            });
          })();
          (_c = document.getElementById("dt-open-tab")) == null ? void 0 : _c.addEventListener("click", () => {
            window2.open("https://www.ducktiles.com/", "_blank", "noopener");
          });
          ...
          copyToClipboard();
          input == null ? void 0 : input.addEventListener("input", async () => {
            try {
          ...
          const escHandler = (evt) => {
            if (evt.key === "Escape") {
              modalEl.style.display = "none";
              window2.removeEventListener("keydown", escHandler);
              if (!IS_MOBILE) {
                this.hidden_input.focus();
              }
            }
          };
          window2.addEventListener("keydown", escHandler);
        }
        openSettings() {
          var settingsHTML = `
