...
(function () {
  ...
  function Puz() {
    ...
    this.circled = function (index) {
      return (
        this.gext !== undefined && (getByte(this.gext, index) & 0x80) !== 0
      );
    };
    this.startDownWord = function (x, y) {
      return (
        (y === 0 || this.isBlack(x, y - 1)) &&
        y < this.height - 1 &&
        !this.isBlack(x, y) &&
        !this.isBlack(x, y + 1)
      );
    ...
    this.getWordNbr = function (x, y) {
      ...
      return direction === this.DIRECTION_ACROSS
        ? this.acrossWordNbrs[index]
        : this.downWordNbrs[index];
    };
  ...
  function parsePuz(bytes) {
    ...
    if (!sanity_check) {
      ...
      throw {
        name: 'BADMAGICNUMBER',
        message:
          "File did not contain expected magic number, contained '" +
          filemagic +
          "'.",
      };
    }
    ...
    retval.strings = string_convert(bytes.substring(strings_offset)).split(
      '\u0000',
      nbrClues + 4
    );
    retval.grid = string_convert(
      bytes.substring(grid_offset, grid_offset + wh)
    );
    ...
    for (y = 0; y < h; y++) {
      for (x = 0; x < w; x++, index++) {
        ...
        downWordNbrs.push(
          sdw ? sqNbr : isBlack || y === 0 ? 0 : downWordNbrs[index - w]
        );
        acrossWordNbrs.push(
          saw ? sqNbr : isBlack || x === 0 ? 0 : acrossWordNbrs[index - 1]
        );
        if (sdw || saw) {
          if (saw) {
      ...
      if (chksum !== compChksum) {
        throw {
          ...
          message:
            'Extra section ' +
            sectName +
            ' had computed checksum ' +
            compChksum +
            ', versus given checksum ' +
            chksum,
        };
      }
      if (sectName === 'GEXT') {
    ...
    // Check for a notepad
    var additional_clues = retval.strings.slice(
      3 + retval.nbrClues,
      4 + retval.nbrClues
    );
    retval.notes = '';
