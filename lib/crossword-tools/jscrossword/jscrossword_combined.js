    ...
    for (var y=0; y < height; y++) {
        for (var x=0; x < width; x++) {
            ...
            var new_cell = {
                ...
                clue: null,
                value: null,
                fixed: cell_attributes.fixed === true,
...
function xw_read_jpz(data1) {
    ...
    //var metadata = {'title': '', 'author': '', 'copyright': '', 'description': '', 'fakeclues': false};
    var metadata = {
        'title': '', 'author': ''
    ...
    if (completion.length) {
        metadata['Way to go!'] = XMLElementToString(completion[0]);
    }
...
<completion friendly-submit="false" only-if-correct="true">GG!</completion>
...
(function () {
    ...
    function parsePuz(bytes) {
        ...
        retval.crossword_type = 'crossword'; // Default

        // Detect DIAGRAMLESS puzzles correctly
        // In .puz files, this happens if:
        //  - grid contains ":" characters (from xroz code) or
        //  - puzzleTypeShort == 0x0401 (diagramless flag)
        if (retval.grid.indexOf(':') !== -1 || puzzleTypeShort === 0x0401) {
        retval.crossword_type = 'diagramless';
        console.log('PUZ metadata: Diagramless puzzle detected!');
        }
